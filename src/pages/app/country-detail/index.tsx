...
import { CountryDetail as CountryDetailInterface } from "@/interfaces/country-detail";
...
export function CountryDetail() {
  ...
  const { data, isLoading } = useQuery<CountryDetailInterface>({
    ...
    queryFn: async () => {
      const response = await fetch(`https://restcountries.com/v3.1/alpha/${countryCode}?fields=name,flags,population,region,subregion,capital,tld,currencies,languages,borders`)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col">
        <h1 className="text-center text-2xl font-semibold">{data?.name.official}</h1>
      ...
      <div className="flex flex-col gap-2">
        ...
        <p><span className="font-semibold">Currencies:</span> {getCurrencies().join(', ')}</p>
        <p><span className="font-semibold">Languages:</span> {getLanguages().join(', ')}</p>
        <p>
          <span className="font-semibold">Borders:</span>{' '}
          {data!.borders.length > 1 ? (
            <>
              {data?.borders.map((border, index) => (
                <Button key={index} variant={'outline'} size={'sm'} className="mr-3" asChild>
                  <Link to={`/countries/${border.toLowerCase()}`} key={index}>{border}</Link>
                </Button>
              ))}
            </>
          ) : (
